      ...
      {"diagnose-slow-query",
       ...
       [](const json& a) -> std::string {
         ...
         s +=
           ...
           "exec.user_time_s plus exec.system_time_s against total_exec_ms is "
           "how much was CPU, and exec.reads_bytes against shared_blks_read "
           "times block_size is how much of what PostgreSQL asked for really "
           "came off storage rather than the page cache. waitEventProfile with "
           "group_by=query and the same query_id names what it waited on over "
           "every execution, where one EXPLAIN ANALYZE shows one run. A "
           "statement that is mostly CPU is a plan or a rewrite question; one "
           "that is mostly IO/DataFileRead with real device reads is a caching "
           "or an index question; one that is mostly Lock is not this prompt's "
           "question at all -- take it to triage-lock-contention.\n"
           ...
           "show. Where the executing role has its own work_mem, plan it again "
           "with plan_as_role set to that role rather than only noting the "
           "difference: work_mem decides whether a sort spills at all, so a "
           "spill that appears under one environment and not the other is the "
           "whole diagnosis, and a spill that appears under both is real. "
           "planning_environment reports what was applied and what was "
           "skipped, so read skipped_from_role before treating the "
           "reproduction as complete.\n"
           "4. If the estimates are wrong, first check that the misestimate "
           "is typical rather than this run's: predicateStats with the "
           "query_id, where pg_qualstats is installed, reports "
           "mean_err_estimate_ratio for every predicate of the statement over "
           "every sampled execution -- the factor by which the row estimate "
           "missed, 0 when it did not. Then call tableStats on the tables "
           ...
           "7. Where pg_qualstats is installed, call suggestIndexes and look "
           "for a suggestion whose query_ids include this statement: its ddl is "
           "an index the extension derived from how the predicates actually "
           "filtered, and it is exactly what evaluateIndex takes in step 9 -- "
           "with, on PostgreSQL 14 and 15, representative values in place of "
           "the statement's $n placeholders, since only 16 can plan them. "
           "Treat it as a candidate, not a verdict -- the advisor does not know "
           "the write cost. Its not_indexable list is predicates no index can "
           "serve, which are rewrite questions. "
           "Before proposing an index, call duplicateIndexes to check that "
      ...
      {"triage-active-sessions",
       ...
       [](const json& a) -> std::string {
         ...
         return
           ...
           "   That split is one sample, taken now. Where pg_wait_sampling is "
           "installed, waitEventProfile gives the same split accumulated over "
           "time -- its null event_type is the no-wait group, on CPU -- which "
           "separates a pattern from a moment. Its counts run from the last "
           "reset, so for the incident's own window read it twice and take the "
           "difference. Its Client/ClientRead includes idle connections waiting "
           "for work, so do not read that entry as the application being slow.\n"
           ...
           "   - Where pg_stat_kcache is installed, statementKernelStats "
           "ordered by exec_cpu_time names the statements spending the CPU as "
           "the kernel counted it, and a high exec.nivcsws -- involuntary "
           "context switches -- is backends being preempted, which is what "
           "running past the core count looks like from inside one.\n"
      ...
      {"explain-and-fix",
       ...
       [](const json& a) -> std::string {
         ...
         return
           ...
           "work_mem, do not stop at saying the plan is wrong -- plan it "
           "again with plan_as_role set to that role. explainQuery applies "
           "the role's planner settings for that one transaction and returns "
           "planning_environment saying exactly what it applied and what it "
           "skipped. THE COMPARISON IS THE FINDING, and it is the same shape "
           "as the generic-versus-custom comparison above: two plans, one "
           "difference, and the difference is the answer. Where they agree, "
           "the environment is not the cause and you can rule it out instead "
           "of carrying it as a caveat.\n"
           "   Read skipped_from_role rather than assuming the role was fully "
           "reproduced. A search_path or a statement_timeout on that role is "
           "reported there and NOT applied -- search_path deliberately, "
           "because it changes which objects the statement resolves to rather "
           "than how they are joined, and planning against a different table "
           "than you meant is worse than not planning at all.\n"
           ...
           "about to run it. With explicit settings it runs only within the "
           "execution budget the connection's declared host capacity allows; if "
           "analyzed comes back false, read planning_environment.execution_budget "
           "for why, and either lower the settings or drop them -- plan_as_role "
           "is not budgeted, because it is production's own environment.\n"
